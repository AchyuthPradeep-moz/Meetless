    ...
    <div className="bg-white border border-gray-200 rounded-lg p-5 hover:shadow-sm transition-shadow cursor-pointer">
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-3">
            <div className="text-gray-900 mb-1 truncate">{meeting.title}</div>
            <div className="text-sm text-gray-600">
              {formatTime(meeting.start_time)} – {endTime(meeting.start_time, meeting.duration)}
              <span className="ml-2 text-gray-400">({meeting.duration} min)</span>
            </div>
          </div>
          ...
          {meeting.reason && (
            <p className="text-sm text-gray-700 mb-3">{meeting.reason}</p>
          )}
            ...
            <div className="flex items-center gap-3">
              ...
              {meeting.confidence != null && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">{meeting.confidence}%</span>
                </div>
              )}
            </div>
